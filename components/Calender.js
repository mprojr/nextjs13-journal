...
export default function Calender(props) {
  ...
  const numRows = (Math.floor(daysToDisplay / 6)) + (daysToDisplay % 7 ? 1 : 0)
        ...
        return (
          <div key={rowIndex} className='grid grid-cols-7 gap-1'>
            {dayList.map((dayOfWeek, dayOfWeekIndex) => {
              ...
              return (
                <div style={{ background: color }} className={'text-sm sm:text-sm border-2 border-solid p-2 flex items-center gap-2 justify-between rounded-md border-black' + (isToday ? ' border-indigo-400' : 'border-indigo-100') + (color === 'white' ? ' text-indigo-400' : 'text-white') } key={dayOfWeekIndex}>
                  <p>{dayIndex}</p>
                </div>
              )
            })}
